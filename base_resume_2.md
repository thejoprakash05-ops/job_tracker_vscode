# Thejo Prakash Karnam Shanmugam
804 Inverness Way, Sunnyvale CA 94087
+1-669-455-7187 | thejoprakash@yahoo.com
https://www.linkedin.com/in/thejoprakash

---

## Technical Skills

- **Programming Languages:** C, C++, Java, Python, PHP, Tcl
- **Machine Learning & AI:** Applied Machine Learning, Deep Learning, Sequence Models, Agentic AI, Embeddings, Vector Databases
- **ML Frameworks:** Scikit-learn, TensorFlow, PyTorch, FAISS
- **Algorithms & Theory:** Data Structures, Algorithms, Combinatorial Optimization, Graph Algorithms
- **Hardware & Architecture:** ASIC, FPGA, CPU Architecture, SoC Design
- **Distributed Systems:** Distributed Systems Design, Meta-scale Infrastructure
- **AI Coding Tools:** GitHub Copilot, Cursor, Claude

---

## Work Experience

### Executive Director, R&D | Synopsys Inc. | Oct 2022 – Present

- **Strategic Product Leadership:** Spearheading HAPS ProtoCompiler R&D strategy to accelerate performance and time-to-market; delivered critical performance and bring-up optimizations that solidified Synopsys as a leader in hardware-assisted verification.
- **AI First Culture:** Established an "AI First" engineering culture, implementing best practices for prompt engineering and context management that improved feature development cycles by 30%.
- **Organizational Management:** Directing a high-impact organization of engineers and managers, fostering a culture of technical excellence to maximize FPGA Synthesis Quality of Results (QoR) and overall system throughput.
- **Cross-Functional Orchestration:** Partnering with AMD and internal PNR (Place and Route) leadership to architect placement-aware synthesis flows, directly improving silicon realization and hardware performance metrics.
- **Enterprise-Wide Influence:** Championing cross-functional initiatives across Product Validation, DevOps, and AE teams; exercising influence over a 100+ engineer ecosystem to align technical roadmaps with business objectives.
- **Executive Stakeholder & Customer Engagement:** Driving strategic customer engagements and roadmap negotiations with Tier-1 semiconductor clients and C-suite management to secure long-term design wins.

### Software Engineering Group Director | Cadence Design Systems | Jul 2018 – Oct 2022

- **Strategic Site Leadership:** Served as the Bangalore Site Lead for the Protium software organization, exercising end-to-end accountability for the ML platform, timing performance, and enterprise-grade scalability.
- **Organizational Building & Culture:** Founded and scaled a high-performance R&D organization from the ground up, recruiting and mentoring a 20-member engineering team while fostering a culture of technical excellence and rapid delivery.
- **Technical Execution & Scalability:** Directed technical strategy and execution for a 50+ engineer ecosystem, successfully re-architecting systems to significantly boost overall system speed and throughput.
- **Revenue Growth & Product Innovation:** Spearheaded the integration of Machine Learning into core products (6+ production models), yielding a 10X revenue increase and accelerating customer adoption through 3–4X improvements in FPGA PNR time.
- **Executive & Stakeholder Management:** Orchestrated high-level roadmap negotiations and strategic engagements with Tier-1 customers and corporate executive leadership to align product evolution with market demand.

### Software Engineer / Project Lead | Facebook Inc., Menlo Park, USA | Oct 2015 – Jul 2018

- **Distributed ML Infrastructure (Feed Recommendations):** Architected and deployed a high-scale vector similarity search platform leveraging FAISS; democratized nearest-neighbor lookup for 30+ ML teams and 100+ engineers, becoming a foundational component for core Facebook product ranking.
- **End-to-End Recommendation Ecosystem:** Served as Project Lead for a full-stack recommendation engine, overseeing the development of high-throughput data pipelines, feature engineering frameworks, and collaborative filtering models for global video discovery.
- **Business Integrity & Risk Mitigation:** Led the design of a K-Nearest Neighbor (KNN) classification system for the Ads Integrity platform; engineered automated ML pipelines that successfully identified and mitigated "bad actor" content, directly protecting platform brand safety and revenue.
- **Cross-Functional Technical Leadership:** Orchestrated the lifecycle of ML models from training to production, integrating NLP techniques and advanced filtering logic to improve the relevance and safety of global feed architectures.
- **Observability & Reliability Engineering:** Partnered closely with SRE on reliability contracts for high-scale ML services; defined SLOs and drove reliability improvements across recommendation and integrity systems, with SRE owning the tooling infrastructure.

### Software Architect / Director | Cadence Design Systems Inc., San Jose, USA | Feb 2014 – Sep 2015

- **Product Performance & Market Penetration:** Architected a 50% breakthrough in system clock speed (100MHz to 150MHz), a pivotal technical milestone that served as the primary catalyst for initial customer acquisition and remains a core competitive differentiator.
- **Multi-Site Technical Directorship:** Exercised full P&L and operational oversight for a geographically distributed R&D organization across San Jose and Noida, aligning global engineering efforts with long-term product roadmaps and corporate strategy.
- **Strategic Team Scaling & Governance:** Spearheaded international talent acquisition and mentorship programs to build a high-caliber engineering team; institutionalized rigorous coding standards and execution frameworks to ensure predictable, high-quality product releases.
- **Customer-Centric Innovation:** Transformed technical performance metrics into business value, directly influencing the sales cycle and securing "design wins" through superior hardware-software co-optimization.

---

## Early Work Experience

### Senior Staff Software Engineer | Tabula Inc., San Jose, USA | Sep 2012 – Feb 2014
- Worked in Global placement and added many improvements such as incremental HFN split, incremental netlist updates, and placement QoR.
- Implemented SAT-based solution for placing and routing clocking components.

### Staff Software Engineer | Xilinx Inc., San Jose, USA | Jan 2011 – Sep 2012
- Implemented optimizations such as decomposition-based rewiring (signal pushing), wide gate signal swap, and fanout optimization.
- Implemented infrastructure to benchmark the results; the framework is used for daily results generation.

### Staff R&D Engineer | Synplicity/Synopsys Inc., Sunnyvale, USA | May 2001 – Jan 2011
- ICC Design Planning (Tech Lead): Worked with customers and AEs to demonstrate 2X improvement in runtime and 3X improvement in quality.
- Logic synthesis support for new Altera/Actel newer devices.

### Software Engineer | Motorola India, Bangalore, India | Feb 2000 – May 2001
- Contributions: SPICE parser, report enhancements.

---

## Education
- **M.Tech in Computer Science** — IIT Kharagpur, India, 1998–2000
- **B.Tech in Electronics & Communications** — S.V. University, Tirupati, India, 1994–1998
